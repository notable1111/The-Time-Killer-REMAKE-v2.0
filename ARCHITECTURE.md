...
- **WalkabilityGrid** — samples the physics world into a walkable/blocked grid at **node points aligned to the world integer lines** (0.5 spacing). Alignment is load-bearing: this map's wall colliders are thin strips sitting ON integer coords, so a coarse grid sampling half-integer cell centers misses them. Blocked = a static non-trigger collider overlaps (ALL layers — walls live on several); dynamic bodies (player/maniac) and triggers (camera zones) filtered out.
- **GridPathfinder** — A* (binary-heap open set + lazy deletion, ~0.2ms/query) + line-of-sight "string pull" → a few natural straight-line world waypoints.
- **ManiacNavigator** (on the maniac, auto-added by ManiacController) — `MoveTo(dest, speed)` paths + feeds `ManiacMotor` one waypoint at a time, repaths on move/interval; falls back to direct on an unreachable target. `Rebuild()` re-samples (call when the layout opens — doors). Tunables: clearance, repath distance/interval, waypoint tolerance.
